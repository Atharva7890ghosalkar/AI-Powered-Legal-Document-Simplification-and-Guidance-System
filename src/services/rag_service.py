...
class RAGService:
    ...
    def _build_prompt(*, question: str, clause_text: str | None, chunks: list[RetrievedChunk]) -> str:
        has_document_context = bool(clause_text and clause_text.strip())
        ...
        return (
            "You are an Indian legal guidance assistant. The user has not uploaded a document.\n\n"
            "Retrieved Legal Context:\n"
            f"{context or '- No retrieved legal context found.'}\n\n"
            f"User Question:\n{question}\n\n"
            "Task Requirements:\n"
            "- Answer as a general legal explainer, not as a document analyzer.\n"
            "- Do not use sections such as Document Metadata, Parties Involved, Effective Date, or Document Duration.\n"
            "- Do not pretend a specific legal document was uploaded.\n"
            "- Explain the topic in simple, practical Indian legal terms.\n"
            "- If the question is broad, first define the topic, then explain common clauses/risks, then give practical cautions.\n"
            "- If context is limited, say so clearly instead of inventing details.\n"
            "- End the response with a section titled 'Follow-up Questions' and include 2-4 useful next questions.\n\n"
            "Use exactly these sections and order:\n\n"
            "1. Quick Answer\n"
